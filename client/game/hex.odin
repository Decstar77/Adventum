...
draw_hex_outline_at :: proc(p: ^Platform, center: [2]f32, thickness: f32, color: [4]f32) {
	corners := hex_corners(center)
	for i in 0 ..< 6 {
		a := corners[i]
		b := corners[(i + 1) % 6]
		p->draw_line(a.x, a.y, b.x, b.y, thickness, color)
	}
}

draw_hex_outline :: proc(p: ^Platform, c: Hex_Coord, thickness: f32, color: [4]f32) {
	draw_hex_outline_at(p, hex_to_pixel(c), thickness, color)
}
