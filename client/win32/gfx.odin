...
gfx_init :: proc(g: ^Graphics, r: ^Renderer) -> bool {
	...
	if !sprites_init(&g.sprites, r) do return false
...
gfx_shutdown :: proc(g: ^Graphics) {
	...
	sprites_shutdown(&g.sprites, g.renderer)
...
gfx_begin :: proc(g: ^Graphics) -> bool {
	...
	sprites_set_scissor(&g.sprites, full)
	sprites_set_view(&g.sprites, {1, 1}, {0, 0})
...
gfx_set_view :: proc(g: ^Graphics, scale, offset: [2]f32) {
	...
	sprites_set_view(&g.sprites, scale, offset)
}
...
gfx_end :: proc(g: ^Graphics) {
	...
	sprites_record(&g.sprites, g.renderer, g.cb)
...
gfx_push_scissor :: proc(g: ^Graphics, x, y, w, h: f32) {
	...
	sprites_set_scissor(&g.sprites, rect)
...
// Pixel-art textures.
gfx_load_texture :: proc(g: ^Graphics, path: string) -> u32 {
	return sprites_load(&g.sprites, g.renderer, path)
}

gfx_texture_size :: proc(g: ^Graphics, tex: u32) -> [2]f32 {
	return sprites_texture_size(&g.sprites, tex)
}
